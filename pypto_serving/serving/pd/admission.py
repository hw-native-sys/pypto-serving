...
class FairByteBudget:
    """FIFO weighted budget for in-flight provider work."""

    def __init__(self, limit: int) -> None:
        if type(limit) is not int or limit < 1:
            raise ValueError("PD in-flight byte limit must be a positive integer")
        self.limit = limit
        self._used = 0
    ...
    async def reserve(self, amount: int):
        if type(amount) is not int or amount < 0:
            raise ValueError("PD transfer byte reservation must be non-negative")
        if amount > self.limit:
            raise PDBackpressureError(
                f"PD transfer requires {amount} bytes but the limit is {self.limit}"
            )
        if amount == 0:
            yield
            return
        ...
        waiter: asyncio.Future[None] = loop.create_future()
        async with self._lock:
            self._waiters.append((amount, waiter))
            self._wake_locked()
        acquired = False
        try:
            await waiter
            ...
            async with self._lock:
                if not acquired:
                    try:
                        self._waiters.remove((amount, waiter))
                    except ValueError:
                        if waiter.done() and not waiter.cancelled():
                            self._used -= amount
                else:
