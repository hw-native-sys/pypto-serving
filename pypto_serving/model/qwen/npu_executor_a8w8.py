...
class Qwen314BA8W8PyptoExecutor(CorePyptoExecutor):
    ...
    def _compile_model(self, model: RuntimeModel) -> _CompiledKernels:
        ...
        l3_buffers = {
            ...
            "decode_active_batch_buffer": torch.empty((1,), dtype=torch.int32).share_memory_(),
    ...
    def _compile_prefill_fwd_callable_a8w8(
        ...
        *,
        ...
        dummy_args = [
            ...
            torch.empty((num_layers, hidden_size), dtype=torch.float32),
            torch.empty((num_layers, hidden_size), dtype=torch.float32),
            torch.empty((num_layers * hidden_size, intermediate_size), dtype=torch.int8),
            torch.empty((num_layers * hidden_size, intermediate_size), dtype=torch.int8),
            torch.empty((num_layers, intermediate_size), dtype=torch.float32),
            torch.empty((num_layers, intermediate_size), dtype=torch.float32),
    ...
    def _stack_decode_weights(cls, layers: list[_KernelLayerWeights]) -> dict[str, torch.Tensor]:
        ...
        weights = {
            ...
            "decode_w_gate_scale":     cat("w_gate_scale"),
            "decode_w_up_scale":       cat("w_up_scale"),
    ...
    def _kernel_layer_weights(cls, layer) -> _KernelLayerWeights:
        ...
        return _KernelLayerWeights(
            ...
            w_gate_scale=cls._shared_tensor(layer.w_gate_scale.float().cpu()),
            w_up_scale=cls._shared_tensor(layer.w_up_scale.float().cpu()),
