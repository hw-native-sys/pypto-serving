...
def test_deepseek_hc_input_builder_shapes_prefill_and_decode():
    ...
    assert torch.equal(decode[:, 6:], decode[:, 0:1].expand_as(decode[:, 6:]))
...
def test_deepseek_prepare_decode_inputs_masks_inactive_write_rows():
    runner, model = _runner_for_prepared_inputs()
    ...
    assert prepared.ori_slot_mapping[0, :6].tolist() == [127, 0, 132, 133, -1, -1]
    assert prepared.hca_cmp_slot_mapping[0, :6].tolist() == [0, -1, -1, -1, -1, -1]
    assert prepared.csa_cmp_slot_mapping[0, :6].tolist() == [31, -1, -1, -1, -1, -1]
    assert prepared.csa_idx_slot_mapping[0, :6].tolist() == [31, -1, -1, -1, -1, -1]
    assert prepared.csa_state_slot_mapping[0, :6].tolist() == [127, 128, 264, 265, -1, -1]
