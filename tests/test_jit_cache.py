...
def test_shared_compiler_cache_across_processes(tmp_path):
    ...
    Scalars are runtime values (SCALAR_SEMANTICS=2), so a different factor reuses
    the same READY artifact and must still dispatch with its own value. A changed
    helper global invalidates the artifact. Earlier generations remain reusable,
    including read-only hits.
    """
