...
from python.core.async_engine import AsyncLLMEngineClient, EngineConfig, TokenOutput
...
def test_async_llm_engine_client_routes_to_least_pending_tokens():
    asyncio.run(_check_async_llm_engine_client_routes_to_least_pending_tokens())


def test_async_llm_engine_client_does_not_double_count_admitted_requests():
    asyncio.run(_check_async_llm_engine_client_does_not_double_count_admitted_requests())


def test_async_llm_engine_client_cleans_up_failed_startup():
    asyncio.run(_check_async_llm_engine_client_cleans_up_failed_startup())


async def _check_async_llm_engine_client_routes_to_least_pending_tokens():
    ...
    assert [fake.config.device_ids for fake in created] == [(0,), (1,)]
...
async def _check_async_llm_engine_client_does_not_double_count_admitted_requests():
...
async def _check_async_llm_engine_client_cleans_up_failed_startup():
...
class _FakeCore:
    ...
    async def add_request(self, request_id: str, prompt: str, config, *, on_queued=None):
