...
def test_deepseek_mtp_prefill_reads_only_selected_owner_outputs():
    ...
    class FakeWorker:
        ...
        def copy_from(self, dst, src, nbytes, *, src_offset=0, worker_id=0):
            # `src_offset` matches pypto.runtime.DistributedWorker.copy_from's real signature
            # (used by `_read_mtp_prefill_pre_hc` for a row-offset device read); this stub does
            # not otherwise care about it, and the assertions below never checked it.
