...
def test_explicit_enablement_preserves_process_policy(tmp_path, monkeypatch):
    """An explicit flag overrides only ``enabled`` on the effective process policy.

    ``pypto.configure_cache()`` settings -- root, read-only, extra source paths
    and the extra fingerprint (which takes part in the artifact key) -- must
    survive ``--use-compile-cache``; rebuilding a policy from the environment
    here would drop them and could validate a stale artifact as a hit.
    """
    import pypto
    ...
    monkeypatch.delenv("PYPTO_CACHE_READONLY", raising=False)
    policy = CacheConfig(
        enabled=False,
    ...
    pypto.configure_cache(policy)
    try:
        jit_fn = _FakeJitFn(MagicMock(spec=DistributedCompiledProgram))
        compiler = _make_compiler()
        compiler.compile("prefill", jit_fn, use_cache=True)
        assert jit_fn.last_config.cache_config == dataclasses_replace(policy, enabled=True)
    finally:
