...
# -----------------------------------------------------------------------------------------------------------
"""Validate and summarize the fixed DSV4 DSpark B16/8K/256 serving profile."""
...
def classify_callables(invocations: list) -> tuple[dict[str, str], dict[str, int]]:
    ...
    if len(ordered_hids) == 3:
        ...
        raise RuntimeError(f"cannot classify DSpark B16 callables: {counts}")
...
def main() -> None:
    ...
    lengths = [int(item["completion_tokens"]) for item in responses]
    ...
    if devices != sorted(set(devices)) or len(devices) != DEVICE_COUNT:
        raise RuntimeError(
            f"expected {DEVICE_COUNT} unique device mappings, got {pid_to_device}"
        )
    ...
    report = f"""# DeepSeek V4 DSpark B16/8K/256 serving profile

- Run: `{args.run_id}`
- Workload: `GBS16 / DP4 / EP16 / TP4 / four requests per DP group / Seq8192 / Output256 / DSpark k=7 / temperature=0`
- Serving categories: `{dict(sorted(categories.items()))}`
- Host STRACE invocations: `{len(selected)}` (`{len(selected) // DEVICE_COUNT}` per rank)
...
Open `serving-strace-swimlane.json` in Perfetto for the combined serving and 16-rank Host view.
"""
