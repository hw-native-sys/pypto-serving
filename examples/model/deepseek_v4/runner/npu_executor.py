...
class DeepSeekV4PyptoExecutor(CorePyptoExecutor):
    ...
    def _compile_model(self, model: RuntimeModel) -> DeepSeekV4CompiledKernels:
        ...
        if self._compile_kernels:
            ...
            lm_head = self._compile_l3_callable(
                "deepseek_v4_lm_head",
                modules["lm_head"].l3_lm_head,
                self._lm_head_dummy_args(model, layout),
            )
    ...
    def _lm_head_dummy_args(self, model: RuntimeModel, layout: DeepSeekV4CacheLayout) -> tuple[torch.Tensor, ...]:
        ...
        return (
            torch.empty(
                (layout.ranks, layout.decode_tokens, model.config.hidden_size),
                dtype=torch.bfloat16,
            ),
            torch.empty(
                (layout.ranks, lm_layout.padded_vocab_per_rank, model.config.hidden_size),
                dtype=torch.bfloat16,
            ),
            torch.empty(
                (layout.ranks, layout.decode_tokens, model.config.vocab_size),
                dtype=torch.float32,
            ),
        )
    ...
    def _layer_common_dummy_tensors(
        ...
        *,
        ...
        values = {
            ...
            "csa_inner_wkv": torch.empty((ranks, hidden, DEEPSEEK_V4_CSA_INNER_OUT_DIM), dtype=torch.bfloat16),
            "csa_inner_wgate": torch.empty((ranks, hidden, DEEPSEEK_V4_CSA_INNER_OUT_DIM), dtype=torch.bfloat16),
    ...
    def _validate_kernel_contract(self, layout: DeepSeekV4CacheLayout) -> None:
        ...
        lm_head_path = self._kernel_dir / "lm_head.py"
        tp_size = _int_constant_from_file(lm_head_path, "TP_SIZE")
        if tp_size is not None and tp_size != layout.ranks:
            raise ValueError(
                f"DeepSeekV4 serving requires lm_head.py TP_SIZE={layout.ranks}, "
                f"but {lm_head_path} has TP_SIZE={tp_size}. Update the pypto-lib "
                "DeepSeekV4 LM head kernel before running 8-NPU generation serving."
            )
